...
function RegisterSide(props) {
  ...
  useEffect(() => {
    if (props.show && firstInput !== null) {
      firstInput.current.focus();
    }
    if (props.show === false) {
      setNameInput({ ...phoneInput, value: '', valid: true, message: '' });
      setFamilyInput({ ...step2Code, value: '', valid: true, message: '' });
      setPhoneInput({ ...step2Code, value: '', valid: true, message: '' });
  ...
  const registerStep1ClickHandler = () => {
    ...
    if (
      ...
      axios
        .post('http://185.211.56.9:5000/User/Register', {
